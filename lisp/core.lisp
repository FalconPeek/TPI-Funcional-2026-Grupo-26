...
(defun recomendacion-ciclo (duracion)
  (cond
    ...
    (t "Recomendado: Ciclo dentro del rango óptimo (35s - 150s).")))
...
(defun distribucion-temporal (horas)
  (if (or (not (numberp horas)) (<= horas 0))
      ...
      (let* ((tiempo-total (* horas 3600))
             ...
             (rojo (+ (* ciclos 90) (min resto 90)))
             (rojo-intermitente (+ (* ciclos 3)
                                    (max 0 (min 3 (- resto 90)))))
             (verde (+ (* ciclos 120)
                       (max 0 (min 120 (- resto 93)))))
             (verde-intermitente (+ (* ciclos 3)
                                    (max 0 (min 3 (- resto 213)))))
             (amarillo (+ (* ciclos 6)
                          (max 0 (min 6 (- resto 216)))))
             (amarillo-intermitente (+ (* ciclos 3)
                                       (max 0 (min 3 (- resto 222))))))

        (list
         (list 'rojo (/ (round (* (/ rojo tiempo-total) 10000)) 100.0))
         (list 'rojo-intermitente (/ (round (* (/ rojo-intermitente tiempo-total) 10000)) 100.0))
         (list 'verde (/ (round (* (/ verde tiempo-total) 10000)) 100.0))
         (list 'verde-intermitente (/ (round (* (/ verde-intermitente tiempo-total) 10000)) 100.0))
         (list 'amarillo (/ (round (* (/ amarillo tiempo-total) 10000)) 100.0))
         (list 'amarillo-intermitente (/ (round (* (/ amarillo-intermitente tiempo-total) 10000)) 100.0))))))
