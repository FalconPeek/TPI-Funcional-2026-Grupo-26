...
;; IMPACTO: No destructiva
;; ========================================================
(defun auditoria (color-inicio &optional (log '()))

  (let* ((estado (cargar-estado))
         (color-actual (if estado
                           (first estado)
                           color-inicio)))

    (sleep 6)

    (guardar-estado color-actual (obtener-timestamp))
    ...
    (let ((color-nuevo (timer (obtener-timestamp))))
      ...
      (cond
        ...
        ((not (equal color-actual color-nuevo))

         (let ((timestamp (obtener-timestamp))
               (registro (list (formatear-timestamp (obtener-timestamp))
                              color-actual
                              color-nuevo)))

           (format t "~% Tiempo ~a: La luz ha cambiado de ~a a ~a"
                   (formatear-timestamp timestamp)
                   color-actual
                   color-nuevo)

           (auditoria color-nuevo (cons registro log))))

        (t
         (informe log)
         (auditoria color-actual log))))))
;; ========================================================
